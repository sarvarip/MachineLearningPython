...
def objective_function(X, y, w, lamb):
    ...
    N = X.shape[0]
    Xw = np.dot(X,w).reshape(-1)
    obj_value = (lamb/2) * np.sum(w*w) + (1/N)*np.sum(np.maximum(0,1-np.multiply(y,Xw)))
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        X = Xtrain[A_t,:]
        y = ytrain[A_t].reshape(-1)
        Xw = np.dot(X,w).reshape(-1)
        A_plus = (y*Xw < 1).astype(bool) #Important to be able to select rows
        X_plus = X[A_plus,:]
        y_plus = y[A_plus]
        rate = 1/(iter*lamb)
        w = (1-rate*lamb)*w + rate/k * np.sum(np.multiply(y_plus[:, None], X_plus),axis=0).reshape([-1, 1])
        denom = np.sqrt(lamb) * np.sqrt(np.sum(w*w))
        w = np.minimum(1, 1/denom) * w
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    predy = pred * ytest[:, None]
    correct = predy > 0
    test_acc = np.average(correct)
...
def main():
    ...
    for key, value in test_acc.items():
        print('%s: test acc = %.4f \n' % (key, value))
    #for key, value in train_obj.items():
        #print('%s: train obj = \n' % (key))
        #print(*value)
