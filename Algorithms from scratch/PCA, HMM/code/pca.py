...
def decompress(Y = np.array([]), M = np.array([])):
    ...
    X_hat = np.dot(Y, M.T) #.T does not work is M is 1D, since transpose of a 1D array is the same
...
def reconstruction_error(orig = np.array([]), decompressed = np.array([])):
    ...
    D = np.max(decompressed.shape)
    error = (1/D)*np.sum(np.square(orig - decompressed))
